...
export default function Home() {
  const { data: results, isLoading, error, refetch } = useTransactionsApi(
    "/api/proxy/transactions?quantity=1",
    1
  );
  ...
  return (
    ...
    >
      <div style={{
        ...
        <Button onClick={refetch} color="green">Refresh</Button>
      </div>
      ...
      <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        {isLoading ? (
          ...
          <StatsCollection
            data={results && Object.entries(results?.amountsByCardType)
              .slice(0, 3)
              .map(([cardType, amount]) => ({
                label: cardType,
